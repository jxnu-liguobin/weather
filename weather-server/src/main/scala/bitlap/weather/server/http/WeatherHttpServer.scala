...
import bitlap.weather.server.gql.{GraphQL, GraphQLRoutes}
...
object WeatherHttpServer extends Http4sDsl[IO] {
  ...
  def service[F[_]: Async](
    ...
    BlazeServerBuilder[F]
      ...
      .withHttpApp(
        (routes(dispatcher, dataProvider, weatherClient) <+> GraphQLRoutes(graphql)).orNotFound
      )
      .resource
